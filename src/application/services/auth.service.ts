...
export class AuthService {
  ...
  async register(dto: RegisterDto, ipAddress: string, device: string): Promise<AuthResponseDto> {
    ...
    const savedAccount = await this.accountPort.createAccount(dto.email, hashedPassword, null);

    let profile: { id: string; name: string };
    try {
      profile = await this.profilePort.createProfile(savedAccount.id, dto.email, dto.name);
  ...
  async login(dto: LoginDto, ipAddress: string, device: string): Promise<AuthResponseDto> {
    const account = await this.accountPort.findByEmail(dto.email);
    if (!account) {
      throw new InvalidCredentialsException();
    }

    const isPasswordValid = await this.hashService.compare(dto.password, account.passwordHash);
    if (!isPasswordValid) {
      throw new InvalidCredentialsException();
    }
  ...
  async verifyToken(token: string): Promise<{ isValid: boolean; userId: string; email: string }> {
    try {
